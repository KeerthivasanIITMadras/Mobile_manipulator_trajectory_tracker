...
class RRTStar:
    ...
    def expand(self, bounds, goal_bias):
        ...
        self.cost.append(0)
        self.rewire(self.nearest_neighbour(len(self.q)-1), bounds)
    
    def rewire(self, q_new_idx, bounds):
        for i in range(len(self.q)):
            if self.metric(self.q[i], self.q[q_new_idx]) < 2 and self.cost[q_new_idx] + self.metric(self.q[i], self.q[q_new_idx]) < self.cost[i] and self.is_path_free(self.q[i], self.q[q_new_idx], bounds):
                self.parent[i] = q_new_idx
                self.cost[i] = self.cost[q_new_idx] + self.metric(self.q[i], self.q[q_new_idx])
    ...
    def connect(self, q_new_idx, q_near_idx, bounds):
        ...
        if self.is_path_free(q_near, q_new, bounds):
            # path_cost = self.cost[q_near_idx] + self.metric(q_near, q_new)
            # near_indices = self.find_near_neighbours(q_new_idx)

            # for idx in near_indices:
            #     if self.cost[idx] + self.metric(self.q[idx], q_new) < path_cost:
            #         q_new_idx = idx
            #         path_cost = self.cost[idx] + self.metric(self.q[idx], q_new)
            self.parent[q_new_idx] = q_near_idx
            # self.cost[q_new_idx] = path_cost
            return True
        return False

    # def find_near_neighbours(self, q_new_idx, radius=1.0):
    #     near_indices = []
    #     for i, q_node in enumerate(self.q):
    #         if self.metric(self.q[q_new_idx], q_node) < radius:
...
def continousTrajectory(initial_config, goal_config):
    ...
    rrt = RRTStar(initial_config, goal_config)
    goal_bias = 0.2
